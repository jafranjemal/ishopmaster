    ...
    if (item.serialized) {
      ...
      stockDetails = await SerializedStock.aggregate([
        ...
        },
        {
          $group: {
            _id: "$item_id",
            totalStock: { $sum: { $cond: [{ $eq: ["$status", "Available"] }, 1, 0] } },
            soldQty: { $sum: { $cond: [{ $eq: ["$status", "Sold"] }, 1, 0] } },
            adjustmentQty: { $sum: { $cond: [{ $eq: ["$status", "Damaged"] }, 1, 0] } },
...
exports.createStock = async (req, res) => {
    try {
      const { item_id, batch_number, purchase_qty, unitCost, sellingPrice, purchaseDate } = req.body;
...
exports.getStockByPurchaseId = async (req, res) => {
  try {
        ...
        $group: {
          ...
          batches: {
            $push: {
              ...
              serialNumber: "$serialNumber",
              purchaseDate: "$purchaseDate",
              availableQty: { $cond: [{ $eq: ["$status", "Available"] }, 1, 0] },
              unitCost: "$unitCost",
              sellingPrice: "$sellingPrice",
            },
          },
    ...
    const mergedStocks = nonSerializedStocks.map((nonSerialized) => ({
      ...nonSerialized,
      profit_margin: ((nonSerialized.sellingPrice - nonSerialized.unitCost) / nonSerialized.sellingPrice) * 100,
    }));
    ...
    const serializedStocks = await SerializedStock.aggregate([
      ...
      },
      {
        $group: {
          ...
          totalStock: { $sum: 1 }, // Count of available serialized items
          lastSellingPrice: { $last: '$sellingPrice' }, // Count of available serialized items
          lastUnitCost: { $last: '$unitCost' }, // Count of available serialized items
          items: {
            $push: {
