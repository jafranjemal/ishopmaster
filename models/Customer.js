...
async function generateCustomerId() {
  const lastCustomer = await this.constructor.findOne().sort({ created_at: -1 });
  let currentId = 0;

  if (lastCustomer && lastCustomer.customer_id) {
    const customerId = lastCustomer.customer_id.replace("CUS-", "");
    if (!isNaN(customerId)) {
      currentId = parseInt(customerId, 10);
    }
  }
