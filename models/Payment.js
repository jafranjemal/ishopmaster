...
const paymentSchema = new mongoose.Schema({
  ...
  references: {
    ...
    agent_id: { type: String, required: false }, // ID of the specific contact/agent who handled the transaction
    agent_name: { type: String, required: false }, // Snapshot of agent name for historical audit (prevents orphaned data if contact deleted)
...
paymentSchema.pre('save', function (next) {
  if (!this.payment_id) { this.payment_id = generatePaymentId('PAY'); }
  next();
});
